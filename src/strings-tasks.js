...
function repeatString(str, times) {
  if (times > 0) {
    return str.repeat(times).replaceAll(' ', '');
  }
  return '';
}
...
function orderAlphabetically(str) {
  return str
    .split('')
    .sort((a, b) => Intl.Collator('en-GB').compare(a, b))
    .join('');
}
...
function countVowels(str) {
  return Array.from(str.toLowerCase()).filter((e) => 'aeiouy'.includes(e))
    .length;
}
...
function findLongestWord(sentence) {
  const arr1 = sentence.split(' ').sort((a, b) => {
...
function extractNameFromTemplate(value) {
  const arr1 = value.replaceAll('!', ' ');
  const arr2 = arr1.split(' ');
  const str = `${arr2[1]} ${arr2[2]}`;
